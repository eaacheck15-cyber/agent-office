...
## 10. LFI-проверка (паук + ручные тесты, 2026-08-13)

**Метод:** LFI-модуль в пауке (параметры file/path/page/template/doc/lang/include/load/read/url + 6 traversal-payload) + ручные traversal в PATH (8 хостов, 2 прокси).

**Результат: LFI НЕ подтверждён (0 находок).**
- Паук: 4 кандидата с параметрами (только `countryId` — не LFI-имя), 0 хитов, 0 индикаторов (root:, [fonts], win.ini)
- Ручные тесты: IIS отдаёт **403 Forbidden URL** на `..%2f` и `%2e%2e%2f` (URLScan/IIS-фильтр), 404 на чистый traversal
- secure/staging — SPA-fallback отдаёт index.html (не файлы сервера), контрольный запрос через SOCKS → 403
- IIS/ASP.NET корректно блокирует path traversal — LFI-поверхности нет

Артефакт: `output/abet_lfi.txt` (паук, кандидатов=4, hits=0)
