...
## 🚨 Shodan-находка: админ-панели инфраструктуры QuoMarkets ТОРЧАТ В ИНТЕРНЕТ (2026-08-13)

Проверено через Shodan/InternetDB (IP 51.68.54.184, OVH):

| Порт | Сервис | Версия | Риск |
|---|---|---|---|
| 2379 | **etcd** (K8s) | — | 🔴 панель кластера без видимой защиты |
| 3000 | **Grafana** | 12.3.2 | 🔴 дашборд мониторинга (часто без пароля) |
| 5672 | **RabbitMQ** | 4.2.0-rc.1 | 🔴 брокер сообщений |
| 7946 | K8s-сеть | — | 🔴 gossip-порт кластера |
| 9000/9443 | **Portainer** | 2.33.2 | 🔴 админ-панель Docker |
| 8080 | HTTP | — | 🟠 |
| 8000 | HTTP | — | 🟠 |

**28 CVE** на этом хосте (CVE-2026-28379, CVE-2026-33375, CVE-2026-44885 и др.)

### Что это значит
- **Portainer 9000/9443** — панель управления Docker-контейнерами скама. Без пароля = полный контроль над инфраструктурой.
- **Grafana 3000** — дашборды, часто открыты с guest-доступом (логи, метрики, данные).
- **RabbitMQ 5672** — брокер сообщений (платёжные события, ордера).
- **etcd 2379** — ключ-хранилище K8s (секреты кластера).

### Всё это — в интернете с внешнего IP (51.68.54.184, OVH)
Не за внутренним VPN, не за фаерволом — просто торчит наружу.

### Артефакт
- `output/shodan_targets.json` — полный отчёт по всем целям
- IP: 51.68.54.184 (OVH, связан с QuoMarkets через разведку)
